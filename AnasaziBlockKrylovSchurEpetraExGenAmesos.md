...
for this problem:
  ...
  1. Use Anasazi to solve Az = σ z, which is a spectral transformation of the original problem K x = λ M x.
  1. The eigenvalues of the original problem are the inverses of the eigenvalues of the transformed problem.

The "operator A such that A z = K^{-1} M z" is a subclass of
Epetra\_Operator.  The Apply method of that operator takes the vector
...
example below uses the sparse direct solver KLU to do so.  Trilinos'
Amesos package has an interface to KLU.

# Code example #

To access the code example from Anasazi's documentation,
[click here.](http://trilinos.sandia.gov/packages/docs/dev/packages/anasazi/doc/html/BlockKrylovSchur_2BlockKrylovSchurEpetraExGenAmesos_8cpp-example.html)
That example is most likely to be up to date, since it passes nightly
tests.  If that example is not up to date or available, you may use
